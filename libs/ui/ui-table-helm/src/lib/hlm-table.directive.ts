...
export class HlmTableDirective {
	...
	constructor() {
		this._tableClassesSettable?.setTableClasses({
			...
			headerRow:
				'flex min-w-full w-fit border-b border-border [&.cdk-table-sticky]:bg-background ' +
			...
			bodyRow:
				'flex min-w-full w-fit border-b border-border transition-[background-color] hover:bg-muted/50 [&:has([role=checkbox][aria-checked=true])]:bg-muted',
		});
	}
}
